...
from tensorflow.keras.layers import (
    Input, Dense, Dropout, BatchNormalization, LSTM, Bidirectional,
    Conv1D, MaxPooling1D, Concatenate, GlobalAveragePooling1D, 
    GlobalMaxPooling1D, TimeDistributed, Activation
)
...
# -------- CHECKPOINT SYSTEM --------
CHECKPOINT_DIR = os.environ.get("NNEDGE_PRIVACY_CHECKPOINT_ROOT", os.path.join(REPO_ROOT, "checkpoints", "Game-3_RNN_LocationAblation_Checkpoints"))
...
def get_or_create_script_id():
    if os.path.exists(CHECKPOINT_DIR):
        ...
        existing_dirs = [d for d in os.listdir(CHECKPOINT_DIR) 
                        if d.startswith("Game-3_RNN_LocationAblation_") and len(d.split('_')) >= 4]
        ...
        for dir_name in existing_dirs:
            ...
            script_id = "_".join(dir_name.split('_')[:5])  # Extract script ID (updated for RNN)
    ...
    new_script_id = f"Game-3_RNN_LocationAblation_{uuid.uuid4().hex[:8]}"
...
def cleanup_empty_checkpoints():
    ...
    for dir_name in os.listdir(CHECKPOINT_DIR):
        if not dir_name.startswith("Game-3_RNN_LocationAblation_"):
            continue
...
def extract_sequences(df):
    ...
    Optimized for RNN architecture with richer temporal features.
    """
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        ...
        if window_data.shape[0] == window_size:
            ...
            # Feature engineering optimized for RNN temporal patterns
            ...
            # 3. Temporal derivative features (rate of change) - critical for RNNs
...
# ---------- MODEL ----------
def build_rnn(input_shape, num_classes, units=[128, 64], dropout_rate=0.3, use_conv=True):
    """
    Enhanced RNN model for multivariate time-series HAR classification.
    Combines CNN preprocessing with bidirectional LSTM layers for better performance.
    ...
    Args:
        ...
        use_conv (bool): Whether to use CNN preprocessing
    ...
    with tf.device('/GPU:0' if tf.config.list_physical_devices('GPU') else '/CPU:0'):
        ...
        # Optional CNN preprocessing for local pattern extraction
        if use_conv:
            x = Conv1D(64, 3, padding='same', activation='relu', name='conv1')(x)
        ...
        # If the last LSTM layer returns sequences, we need global pooling
        if len(x.shape) == 3:  # (batch, timesteps, features)
            ...
            global_max = GlobalMaxPooling1D(name='global_max_pool')(x)
            x = Concatenate(name='combine_global')([global_avg, global_max])
        ...
        model = Model(inputs, outputs, name='Enhanced_RNN_HAR')
        ...
        print(f"🏗️ Enhanced RNN model built on device: {'GPU' if tf.config.list_physical_devices('GPU') else 'CPU'}")
        print(f"📊 Model parameters: {model.count_params():,}")
        print(f"🎯 Input shape: {input_shape}, Output classes: {num_classes}")
        print(f"🔧 Architecture: {len(units)} LSTM layers {units}, CNN preprocessing: {use_conv}")
...
def train_model_with_checkpoints(model, X_train, y_train_cat, X_test, experiment_id, epochs=EPOCHS, batch_size=BATCH_SIZE):
    ...
    # Minimal data augmentation optimized for time series
    def augment_har_sequences(X, y, shift_range=2, noise_std=0.01):
        X_aug, y_aug = [], []
        for i in range(len(X)):
            X_aug.append(X[i])
            y_aug.append(y[i])
            
            # Time shifting augmentation (good for RNNs)
            if np.random.random() < 0.4:  # 40% chance for augmentation
            ...
            # Small noise augmentation for robustness
    ...
    with tf.device(device_name):
        ...
        if start_epoch < epochs:
            ...
            history = model.fit(
                ...
                verbose=1, 
                callbacks=callbacks, 
                validation_split=0.15,  # Validation split for RNN
                shuffle=True,
...
print("🚀 Starting Game-3 RNN Location Ablation Experiments")
print("=" * 70)
# run_lo_location()
