...
def load_paired_data(data_path, paired_data_root="./save_mmbind"):

	folder_path = os.path.join(paired_data_root, data_path)

	x1 = []
	x2 = []
	# y = np.load(folder_path + "/label.npy")
	similarity = np.load(os.path.join(folder_path, "similarity.npy"))

	for sample_id in range(similarity.shape[0]):

		x1.append(np.load(os.path.join(folder_path, "skeleton", "{}.npy".format(sample_id))))
		x2.append(np.load(os.path.join(folder_path, "gyro", "{}.npy".format(sample_id))))
...
def load_all_paired_data(paired_data_root="./save_mmbind"):

	x1_A, x2_A, y_A = load_paired_data("train_skeleton_paired_AB", paired_data_root=paired_data_root)
	x1_B, x2_B, y_B = load_paired_data("train_gyro_paired_AB", paired_data_root=paired_data_root)
