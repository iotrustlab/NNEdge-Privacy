...
from tensorflow.keras.layers import (
    ...
    Input, GlobalAveragePooling1D, Activation, SeparableConv1D, Add
)
...
CHECKPOINT_DIR = os.environ.get("NNEDGE_PRIVACY_CHECKPOINT_ROOT", os.path.join(REPO_ROOT, "checkpoints", "Game-2_CNN_LocationAblation_Checkpoints"))
...
def get_or_create_script_id():
    if os.path.exists(CHECKPOINT_DIR):
        ...
        existing_dirs = [d for d in os.listdir(CHECKPOINT_DIR) 
                        if d.startswith("Game-2_CNN_LocationAblation_") and len(d.split('_')) >= 4]
    ...
    new_script_id = f"Game-2_CNN_LocationAblation_{uuid.uuid4().hex[:8]}"
...
def cleanup_empty_checkpoints():
    ...
    for dir_name in os.listdir(CHECKPOINT_DIR):
        if not dir_name.startswith("Game-2_CNN_LocationAblation_"):
            continue
...
def build_cnn(input_shape, num_classes):
    """Enhanced CNN architecture for HAR with motion-aware processing."""
    with tf.device('/GPU:0' if tf.config.list_physical_devices('GPU') else '/CPU:0'):
        model = Sequential([
            # Use Input layer first for Keras 3 compatibility
            tf.keras.layers.Input(shape=input_shape),
            
            # Motion-aware feature processing
            Dense(128, activation='relu', name='motion_aware_expansion'),
            BatchNormalization(),
            Dropout(0.2),
            
            # Dual-pathway processing for static vs dynamic patterns
            # Pathway 1: Fine-grained static pattern detection
            Conv1D(32, 3, padding='same', activation='relu', name='static_conv1'),
            BatchNormalization(),
            Conv1D(32, 5, padding='same', activation='relu', name='static_conv2'),
            BatchNormalization(),
            
            # Pathway 2: Gross movement pattern detection  
            Conv1D(32, 7, padding='same', activation='relu', name='dynamic_conv1'),
            BatchNormalization(),
            Conv1D(32, 9, padding='same', activation='relu', name='dynamic_conv2'),
            BatchNormalization(),
            
            # Combine pathways for comprehensive pattern analysis
            Conv1D(64, 5, padding='same', activation='relu', name='combined_conv1'),
            BatchNormalization(),
            MaxPooling1D(2, name='pool1'),
            Dropout(0.25),
            
            # Motion transition pattern detection
            Conv1D(128, 3, padding='same', activation='relu', name='transition_conv1'),
            BatchNormalization(),
            Conv1D(128, 5, padding='same', activation='relu', name='transition_conv2'),
            BatchNormalization(),
            SeparableConv1D(128, 7, padding='same', activation='relu', name='transition_separable'),
            BatchNormalization(),
            MaxPooling1D(2, name='pool2'),
            Dropout(0.3),
            
            # Activity-specific pattern extraction (considers motion context)
            Conv1D(256, 3, padding='same', activation='relu', name='activity_conv1'),
            BatchNormalization(),
            Conv1D(256, 5, padding='same', activation='relu', name='activity_conv2'),
            BatchNormalization(),
            Dropout(0.35),
            
            # High-level motion-activity integration
            Conv1D(512, 3, padding='same', activation='relu', name='integration_conv'),
            BatchNormalization(),
            Dropout(0.4),
            
            # Global temporal pooling for final activity classification
            GlobalAveragePooling1D(name='global_avg_pool'),
            
            # Motion-aware dense classification layers
            Dense(512, activation='relu', name='motion_dense1'),
            BatchNormalization(),
            Dropout(0.5),
            
            Dense(256, activation='relu', name='motion_dense2'),
            BatchNormalization(),
            Dropout(0.4),
            
            # Activity classification head
            Dense(128, activation='relu', name='activity_dense'),
            BatchNormalization(),
            Dropout(0.3),
            
            # Output layer for activity classification
            Dense(num_classes, activation='softmax', name='activity_output')
        ])
        
        # Use advanced optimizer with motion-aware settings
        optimizer = Adam(
        ...
        print(f"🏗️ Motion-aware HAR CNN model built on device: {'GPU' if tf.config.list_physical_devices('GPU') else 'CPU'}")
        print(f"📊 Model parameters: {model.count_params():,}")
        print(f"🎯 Input shape: {input_shape}")
        print(f"🏷️ Output classes: {num_classes}")
        print(f"🎮 Motion context: Utilizes binary motion indicator (dec_tree_out_1: 1=moving, 0=static)")
    ...
    for modality, features in MODES.items():
        ...
        for test_loc in locations:
            ...
            for user in data_by_loc.get(test_loc, {}):
                ...
                model = build_cnn(X_tr.shape[1:], y_tr_cat.shape[1])
