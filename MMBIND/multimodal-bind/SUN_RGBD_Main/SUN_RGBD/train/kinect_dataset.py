...
DEFAULT_SUNRGBD_RAW_ROOT = os.path.join(
    os.environ.get(
        "SUNRGBD_RAW_ROOT",
        os.path.join(
...
DEFAULT_SUNRGBD_KINECT_ROOT = os.path.join(DEFAULT_SUNRGBD_RAW_ROOT, "kv2", "kinect2data", "")

class KinectDataset(Dataset):
    def __init__(self, path = DEFAULT_SUNRGBD_KINECT_ROOT, transform = None):
        self.file_names = []
        self.labels = []
...
class SUNRGBD(Dataset):
    def __init__(self, path = DEFAULT_SUNRGBD_RAW_ROOT, transform=None):
        self.data = []
        if transform:
            self.transform = transform
        else:
