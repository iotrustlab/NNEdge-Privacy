...
from tensorflow.keras.optimizers import Adam  # Updated for Keras 3 compatibility
...
BATCH_SIZE = 32  # Optimized batch size for GPU memory
LEARNING_RATE = 0.001
...
CHECKPOINT_DIR = os.environ.get("NNEDGE_PRIVACY_CHECKPOINT_ROOT", os.path.join(REPO_ROOT, "checkpoints", "Game-1_CNN_LocationAblation_Checkpoints"))
...
def get_or_create_script_id():
    if os.path.exists(CHECKPOINT_DIR):
        ...
        existing_dirs = [d for d in os.listdir(CHECKPOINT_DIR) 
                        if d.startswith("Game-1_CNN_LocationAblation_") and len(d.split('_')) >= 4]
    ...
    new_script_id = f"Game-1_CNN_LocationAblation_{uuid.uuid4().hex[:8]}"
...
def cleanup_empty_checkpoints():
    ...
    for dir_name in os.listdir(CHECKPOINT_DIR):
        if not dir_name.startswith("Game-1_CNN_LocationAblation_"):
            continue
...
def extract_sequences(df):
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        ...
        if window.shape[0] == window_size:
            ...
            window_size_var = min(5, len(binary_seq))  # Smaller for CNN
            variances = []
            ...
            # Create enhanced feature vector for CNN processing
...
def build_cnn(input_shape, num_classes):
    """Enhanced CNN architecture for binary HAR with improved feature extraction."""
    with tf.device('/GPU:0' if tf.config.list_physical_devices('GPU') else '/CPU:0'):
        model = Sequential([
            # Use Input layer first for Keras 3 compatibility
            tf.keras.layers.Input(shape=input_shape),
            
            # Feature expansion for richer binary representation
            Dense(32, activation='tanh', name='feature_expansion'),
            
            # Initial convolution block with larger kernel for pattern capture
            Conv1D(64, 7, padding='same', name='initial_conv'),
            BatchNormalization(),
            Activation('relu'),
            
            # First residual-inspired block
            ...
            # Third block for high-level features
            Conv1D(256, 3, padding='same'),
            BatchNormalization(),
            Activation('relu'),
            SeparableConv1D(256, 3, padding='same'),  # More efficient convolution
            BatchNormalization(),
            Activation('relu'),
            
            # Attention-like mechanism using Global Average Pooling
        ...
        print(f"🏗️ Enhanced CNN model built on device: {'GPU' if tf.config.list_physical_devices('GPU') else 'CPU'}")
...
def train_model_with_checkpoints(model, X_train, y_train_cat, X_test, experiment_id, epochs=EPOCHS, batch_size=BATCH_SIZE):
    ...
    # Data augmentation: time-shift and noise injection for binary sequences
    def augment_binary_sequences(X, y, shift_range=3, noise_prob=0.05):
        X_aug, y_aug = [], []
        for i in range(len(X)):
            ...
            # Noise injection (flip some binary values)
            if np.random.random() < 0.3:  # 30% chance to add noise
                X_noisy = X[i].copy()
                noise_mask = np.random.random(X_noisy.shape) < noise_prob
                X_noisy[noise_mask] = 1 - X_noisy[noise_mask]  # Flip binary values
