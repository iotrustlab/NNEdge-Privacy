"""
Random Forest Training Script for Cross-User HAR Privacy Analysis

Modified Version Features:
1. Randomly selects only 10 combinations per window size and split configuration
...
from sklearn.metrics import (
    classification_report, accuracy_score, confusion_matrix, mutual_info_score, precision_score, recall_score, f1_score
)
...
for window_size in window_sizes:
    ...
    results_root = get_results_root("Game-1", "RF", f"Window_{window_size}")
    os.makedirs(results_root, exist_ok=True)
    cache_path = os.path.join(results_root, "user_data_random_forest.pkl")
    ...
    # ---------- COMBINATORIAL SPLITS (LIMITED TO 10 RANDOM COMBOS) ----------
    ...
    for n_train, n_test in split_configs:
        ...
        for idx, train_users in enumerate(selected_combos):
            ...
            # Save confusion matrix for the first combination only
    ...
    print(f"✅ Random Forest training for window size {window_size} completed in {time.time() - start_time:.2f} seconds.")
