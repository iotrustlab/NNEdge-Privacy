"""
Game-2 Decision Tree Script with Sensor Location Ablation Study
- (Game-2 setting)
...
def extract_features(df, selected_features):
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        win = df[selected_features].iloc[i:i + window_size]
...
def load_data_by_location(modality_name, selected_features):
    data = {loc: {} for loc in locations}
    print(f"🗂️ Starting data loading for modality '{modality_name}'...")
                ...
                for file in files:
                    ...
                    try:
                        ...
                        if df[selected_features].isnull().any().any():
                            print(f"   ⚠️ Null values found in selected columns of {file}")
                            continue

                        f = extract_features(df, selected_features)
                        if f.size == 0:
...
def run_game2_lo_location(modality_name, features, output_tag):
    data_by_loc = load_data_by_location(modality_name, features)
    output_root = os.path.join(data_root, "Results", game, output_tag, "leave-one-location-out")
    ...
    for test_loc in locations:
        ...
        for train_ratio, test_ratio in ratios:
            ...
            if X_train and X_test:
                ...
                model = DecisionTreeClassifier(random_state=42).fit(scaler.transform(X_train), y_train)
        ...
        for user in data_by_loc.get(test_loc, {}):
            ...
            model = DecisionTreeClassifier(random_state=42).fit(scaler.transform(X_tr), y_tr)
...
def run_game2_train_one_test_others(modality_name, features, output_tag):
    data_by_loc = load_data_by_location(modality_name, features)
    output_root = os.path.join(data_root, "Results", game, output_tag, "train-one-test-others")
    os.makedirs(output_root, exist_ok=True)
...
def run_game2_experiments():
    for modality_name, features in modalities.items():
        print(f"\n🔬 Running Game-2 for modality: {modality_name}")
        output_tag = os.path.join(model_name, modality_name)
        run_game2_lo_location(modality_name, features, output_tag)
        run_game2_train_one_test_others(modality_name, features, output_tag)



run_game2_experiments()
