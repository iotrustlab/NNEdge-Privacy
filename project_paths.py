#!/usr/bin/env python3
"""Shared configuration helpers for the Identity + Placement experiments."""
...
SUPPORTED_SPLITS = ("8_3",)
DEFAULT_SPLIT = "8_3"
DEFAULT_RANDOM_SEED = 42

REPO_ROOT = Path(__file__).resolve().parent
DEFAULT_DATA_ROOT = REPO_ROOT / "Data"
DEFAULT_RESULTS_ROOT = REPO_ROOT / "Results" / "Identity-placement"


@dataclass(frozen=True)
class IdentityPlacementExperimentConfig:
    data_root: Path
    results_root: Path
    frequency_hz: int
    split_name: str
...
def build_experiment_arg_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--split",
        default=os.environ.get("NNEDGE_PRIVACY_SPLIT", DEFAULT_SPLIT),
        choices=SUPPORTED_SPLITS,
        help="Train/test split to evaluate for this branch.",
    )
    parser.add_argument(
        "--max-combinations",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Optional override for the results root. Defaults to repo-local Results/Identity-placement/.",
    )
    return parser


def resolve_experiment_config(description: str) -> IdentityPlacementExperimentConfig:
    ...
    return IdentityPlacementExperimentConfig(
        data_root=data_root,
...
def build_model_results_dir(results_root: Path, frequency_hz: int, experiment_name: str, model_label: str) -> Path:
    results_dir = Path(results_root) / f"{frequency_hz}Hz" / experiment_name / model_label
    results_dir.mkdir(parents=True, exist_ok=True)
    return results_dir
