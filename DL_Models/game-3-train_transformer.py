...
results_root = get_results_root("Game-3", "Transformer")
...
def build_transformer(input_shape, num_classes):
    ...
    # Game-3 transformer architecture
    x_proj = Dense(128)(x)
    attention_output = MultiHeadAttention(num_heads=4, key_dim=128)(x_proj, x_proj)
...
import itertools
import random
user_cache_path = os.path.join(results_root, "user_data_all.pkl")
...
for window_size in window_sizes:
    ...
    for n_train, n_test in split_configs:
        ...
        for idx, train_users in enumerate(combos):
            ...
            X_train_trans = X_train.reshape((X_train.shape[0], X_train.shape[1], len(FEATURES)))
            X_test_trans = X_test.reshape((X_test.shape[0], X_test.shape[1], len(FEATURES)))
            model = build_transformer((window_size, len(FEATURES)), len(le.classes_))
            model.fit(X_train_trans, to_categorical(y_train_enc), epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=0)
            y_proba = model.predict(X_test_trans, verbose=0)
