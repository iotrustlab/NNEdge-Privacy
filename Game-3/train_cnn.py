# Combined Features CNN Script (No Modes Loop) with Logging of Skipped Files
...
results_root = os.path.join(data_root, "Results", "Game-3", "CNN")
sequence_length = 100
...
# -------------------- MAIN PIPELINE --------------------
print(f"\n🔧 Using combined features: {FEATURES}\n")
user_data = {}
all_labels = []
...
for user in get_user_dirs(data_root):
    ...
    for file_path in file_paths:
        ...
        if df[FEATURES].isnull().values.any():
            print(f"   ⚠️ Skipping file (NaN values in features): {file_path}")
            continue
...
# ---- CROSS-USER SPLITS ----
cross_user_path = os.path.join(results_root, "cross-user")
os.makedirs(cross_user_path, exist_ok=True)

for tr, ts in train_test_ratios:
    split_name = f"train{int(tr * 100)}_test{int(ts * 100)}"
    split_path = os.path.join(cross_user_path, split_name)
