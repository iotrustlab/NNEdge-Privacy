...
results_root = get_results_root("Game-2", "Transformer")
    ...
    for window_size in window_sizes:
        ...
        for n_train, n_test in split_configs:
            ...
            for idx, train_users in enumerate(combos):
                ...
                model = build_transformer((X_train.shape[1], X_train.shape[2]), len(le.classes_))
                model.fit(X_train, to_categorical(y_train), epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=0)
                y_proba = model.predict(X_test, verbose=0)
                y_pred = np.argmax(y_proba, axis=1)
                metrics = save_results(split_path, y_test, y_pred, le, y_proba)
