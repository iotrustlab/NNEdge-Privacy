        ...
        for activity in activities_to_search:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    # Debug: Show available columns for the first file
                    ...
                    if not missing_features:
                        ...
                        if placement_name == "right-wrist":  # Only log for first placement
                            print(f"        ⚠️ Missing required features in {filename}: {missing_features}")
...
def align_and_stack(placement_dfs, window_size, features=None):
    ...
    if len(aligned_data) > 0:
        ...
        # Verify shape: should be (T, P, C) where P=5 placements, C=expected features
...
def is_multi_placement_game(game_name, features, grid_type='single'):
    """Check if this is a multi-placement game"""
    # Game-1 is multi-placement when used with --grid multi
    # Other games are multi-placement if they have multiple dec_tree_out features OR if grid_type is 'multi'
    return (game_name == "Game-1" or 
            grid_type in ["multi", "mini_v2"] or
            any("dec_tree_out_" in f and "_" in f and f != "dec_tree_out_1" for f in features))


def load_data_for_game(game_name, data_root, num_classes=None, grid_type='single'):
    ...
    # Check if this is a multi-placement game
    is_multi = is_multi_placement_game(game_name, features, grid_type)
    
    # Get activities list - filter by num_classes if specified
    if num_classes and num_classes < len(VALID_ACTIVITIES):
        # Use first N activities in alphabetical order to match LabelEncoder behavior
        activities_to_process = sorted(list(VALID_ACTIVITIES))[:num_classes]
    ...
    for user in user_dirs:
        ...
        if X_user:
            ...
            # Log first user's data shape for feature verification
            if len(user_data) == 1:  # First user
                sample_shape = np.array(X_user).shape
                print(f"    🔍 First user data shape: {sample_shape}")
                if len(sample_shape) == 4:  # Multi-placement: (samples, time, placements, features)
                    print(f"    📊 Multi-placement format: {sample_shape[0]} samples, {sample_shape[1]} time steps, {sample_shape[2]} placements, {sample_shape[3]} features")
                elif len(sample_shape) == 3:  # Single-placement: (samples, time, features)
                    print(f"    📊 Single-placement format: {sample_shape[0]} samples, {sample_shape[1]} time steps, {sample_shape[2]} features")
                print(f"    🎯 Expected features: {features}")
                print(f"    ✅ Feature count matches: {len(features)} expected, {sample_shape[-1]} in data")
...
def prepare_cross_user_data(user_data, train_users, test_users, use_per_user_norm=False, game_name=None, 
    ...
    if featureset != 'base' and featureset != 'concatenated':
        ...
        # For concatenated featureset, we flatten the feature dimensions
        # This should handle both binary (T, P, 1) and multi-feature (T, P, C) cases
        if len(X_train.shape) == 4:  # (samples, time, placements, features)
            num_samples, time_steps, num_placements, num_features = X_train.shape
            print(f"  📊 Multi-feature data: {num_features} features per placement")
            # Reshape to (samples, time, placements * features)
            X_train = X_train.reshape(num_samples, time_steps, num_placements * num_features)
            X_test = X_test.reshape(X_test.shape[0], X_test.shape[1], num_placements * num_features)
            print(f"  📊 Concatenated: {num_placements} × {num_features} = {num_placements * num_features} features")
        elif len(X_train.shape) == 3:  # (samples, time, placements) - already flattened or single feature
            # This is the traditional case - just keep as is
            print(f"  📊 Data already in correct format for concatenation")
        else:
            raise ValueError(f"Unexpected data shape for concatenation: {X_train.shape}")
            
        print(f"  📊 Final training data shape: {X_train.shape} (concatenated)")
        print(f"  📊 Final test data shape: {X_test.shape} (concatenated)")
