                    ...
                    for smoothing in label_smoothing:
                        for num_classes in classes:
                            for seed in seeds:
                                ...
                                # Determine model type based on featureset and game
                                if featureset == 'concatenated':
                                    # Use existing models with concatenated input shape
                                    model_type = 'advanced_cnn'  # Can also use 'lstm_attention', 'multimodal_transformer', etc.
                                elif games[0] == "Game-1":
                                    fusion_to_model = {
                                        'late': lambda f: 'binary_late_fusion',
                                        'weighted': lambda f: 'binary_weighted_fusion',
...
def run_single_experiment(
    ...
    **kwargs
    ...
    try:
        ...
        # Use existing models with concatenated input shape for concatenated featureset
        if featureset == 'concatenated':
            model_type = 'advanced_cnn'  # Can also use 'lstm_attention', 'multimodal_transformer', etc.
        # Use binary models for Game-1 (binary-only), continuous models for other games
...
def run_experiment_grid(
    ...
    **kwargs
    ...
    for i, exp_config in enumerate(experiments):
        ...
        # Save confusion matrix if we have predictions
