...
MMFI_DATASET_ROOT = os.environ.get(
    "MMFI_DATASET_ROOT",
    os.path.join(
...
DEFAULT_SKELETON_AE_WEIGHT = os.environ.get(
    "MMFI_SKELETON_AE_WEIGHT",
    os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "save_imagebind",
        "save_train_AB_skeleton_AE",
        "models",
        "single_train_AB_lr_0.001_decay_0.0001_bsz_64",
        "last.pth",
    ),
)
...
def set_loader(opt):
    ...
    # train_datasetA, _ = make_dataset(MMFI_DATASET_ROOT, config_train)

    with open('../Configs/config_train_B.yaml', 'r') as handle:
        config_train = yaml.load(handle, Loader=yaml.FullLoader)

    train_datasetB, _ = make_dataset(MMFI_DATASET_ROOT, config_train)
...
def set_model(opt):
    ...
    state_dict = torch.load(DEFAULT_SKELETON_AE_WEIGHT)['model']
    filtered_dict = {}
