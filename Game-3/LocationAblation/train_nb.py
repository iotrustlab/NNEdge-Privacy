...
def extract_features(df):
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        win = df[features_to_use].iloc[i:i + window_size]
...
def load_data_by_location():
    ...
    print("\U0001F5C2️ Starting data loading...")
        ...
        for activity in activities:
            ...
            for loc in locations:
                ...
                for file in files:
                    ...
                    try:
                        df = pd.read_csv(file)
                        if not all(f in df.columns for f in features_to_use):
...
def train_and_predict(X_train, y_train, X_test):
    scaler = QuantileTransformer(output_distribution='normal', random_state=42).fit(X_train)
    X_train_scaled = scaler.transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    selector = VarianceThreshold(threshold=1e-4).fit(X_train_scaled)
        ...
        for loc in test_locs:
            for user in data_by_loc[loc]:
                ...
                if not X_train:
                    continue
                X_train = np.vstack(X_train)
                y_pred = train_and_predict(X_train, y_train, X_te)
                intra_preds.extend(y_pred)
                intra_trues.extend(y_te)
