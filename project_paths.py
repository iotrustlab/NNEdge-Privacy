#!/usr/bin/env python3
"""Shared configuration helpers for the Identity experiments."""
...
DEFAULT_SPLIT_NAME = "8_3"
SUPPORTED_SPLIT_NAMES = (DEFAULT_SPLIT_NAME,)
SUPPORTED_MODEL_LABELS = ("RF", "DT", "NB", "CNN", "RNN", "Transformer")

REPO_ROOT = Path(__file__).resolve().parent
DEFAULT_DATA_ROOT = REPO_ROOT / "Data"
DEFAULT_RESULTS_ROOT = REPO_ROOT / "Results" / "Identity"


@dataclass(frozen=True)
class IdentityExperimentConfig:
    data_root: Path
    results_root: Path
    frequency_hz: int
...
def _parse_env_model_labels() -> tuple[str, ...]:
    raw_value = os.environ.get("NNEDGE_PRIVACY_MODEL_LABELS")
    if raw_value is None:
    ...
    invalid_labels = [label for label in model_labels if label not in SUPPORTED_MODEL_LABELS]
    if invalid_labels:
        raise ValueError(
            "Environment variable NNEDGE_PRIVACY_MODEL_LABELS contains unsupported values: "
            f"{invalid_labels}. Supported labels are {SUPPORTED_MODEL_LABELS}."
        )

    return model_labels


def build_identity_arg_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional override for the results root. Defaults to repo-local Results/Identity/.",
    )
    parser.add_argument(
        "--split-name",
        default=os.environ.get("NNEDGE_PRIVACY_SPLIT_NAME", DEFAULT_SPLIT_NAME),
        choices=SUPPORTED_SPLIT_NAMES,
        help="Identity branch split ratio to evaluate. This branch targets 8_3 only.",
    )
    parser.add_argument(
        "--models",
        nargs="+",
        default=list(_parse_env_model_labels()),
        choices=SUPPORTED_MODEL_LABELS,
        help="Model labels to evaluate for the Identity branch sweep.",
    )
    return parser


def resolve_identity_experiment_config(description: str) -> IdentityExperimentConfig:
    parser = build_identity_arg_parser(description)
    ...
    if not args.models:
        parser.error("--models must include at least one model label.")

    results_root.mkdir(parents=True, exist_ok=True)

    return IdentityExperimentConfig(
        data_root=data_root,
        ...
        model_labels=tuple(dict.fromkeys(args.models)),
    )
