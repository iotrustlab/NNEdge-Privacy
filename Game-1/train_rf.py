...
results_root = os.path.join(data_root, "Results", "Game-1", "RF")
window_size = 20
...
def extract_features(df):
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        window = df[['dec_tree_out_1']].iloc[i:i + window_size]
...
# ---------- TRAINING SCENARIOS ----------
print("\n📊 FINAL SUMMARY:")
print(f"Total users with data: {len(user_data)}")
for user, (X, y) in user_data.items():
    print(f" - {user}: {X.shape[0]} samples, labels: {set(y)}")
...
if all_preds:
    ...
    with open(os.path.join(intra_path, "summary.txt"), "w") as f:
        f.write(f"Intra-user Combined Accuracy: {acc_total:.4f}\n\n{report_all}")
...
for test_user in user_data:
    ...
    with open(os.path.join(loo_path, f"report_{test_user}.txt"), "w") as f:
        f.write(f"Leave-One-Out: Test={test_user}\nTrain={train_users}\n\n{report}")
...
if all_preds:
    ...
    report_loo = classification_report(all_trues, all_preds, digits=4, zero_division=0)
    labels_sorted = sorted(list(set(all_trues) | set(all_preds)))
    metrics = compute_leakage_metrics(all_trues, all_preds, labels_sorted)
    with open(os.path.join(loo_path, "summary.txt"), "w") as f:
        f.write(f"LOUO Combined Accuracy: {acc_loo:.4f}\n\n{report_loo}")
...
print(f"✅ Random Forest training & evaluation completed in {time.time() - start_time:.2f} seconds.")
