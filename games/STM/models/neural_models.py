...
def build_advanced_cnn(input_shape, num_classes, game_name, featureset='base', use_film=False,
    ...
    # Handle different input shapes based on featureset and data mode
    if featureset == 'concatenated':
        # Input shape: (100, num_features) - concatenated features from multiple placements
        if len(input_shape) != 2:
            raise ValueError(f"Expected concatenated input shape (100, num_features), got {input_shape}")
        print(f"   📊 Using concatenated input: {input_shape[0]} time steps, {input_shape[1]} features")
        is_multi_sensor = False
    elif len(input_shape) == 3 and input_shape[1] > 1 and input_shape[2] == 1:
        # Multi-sensor input shape: (100, 5, 1) - time, sensors, features
        print(f"   📊 Detected multi-sensor input: {input_shape[0]} time steps, {input_shape[1]} sensors, {input_shape[2]} features")
        is_multi_sensor = True
    else:
        # Standard single-sensor input shape: (100, features) or (100, 1)
        print(f"   📊 Using standard single-sensor input: {input_shape}")
        is_multi_sensor = False
    ...
    # Handle multi-sensor vs single-sensor processing
    if is_multi_sensor:
        # Multi-sensor path: reshape from (time, sensors, features) to (time, sensors*features)
        print(f"   🔧 Reshaping multi-sensor data from {input_shape} to ({input_shape[0]}, {input_shape[1] * input_shape[2]})")
        x = Reshape((input_shape[0], input_shape[1] * input_shape[2]))(inputs)
    else:
        # Single-sensor path: use inputs directly
        x = inputs
...
def build_multimodal_transformer(input_shape, num_classes, game_name, featureset='base', use_film=False,
    ...
    # Handle different input shapes - check if we have 3D input (time, placements, features)
    if len(input_shape) == 3:  # (time, placements, features)
        # Reshape 3D to 2D by flattening placement and feature dimensions
        x = Reshape((input_shape[0], input_shape[1] * input_shape[2]))(inputs)
        input_features = input_shape[1] * input_shape[2]
    else:  # (time, features)
        x = inputs
        input_features = input_shape[1]

    # Feature embedding for multimodal data - dynamically adapt to input size
    print(f"   📊 Input features detected: {input_features}")
    
    if game_name.startswith("Game-2"):
        # Game-2-accel/Game-2-gyro: sensor + binary features
        # For concatenated: 4 features * 5 placements = 20 features
        if input_features >= 15:  # Multi-placement concatenated (e.g., 20 features)
            x = Dense(128, activation='relu')(x)
            x = Dense(64, activation='relu')(x)
            d_model = 64
        else:  # Single placement (4 features)
            x = Dense(64, activation='relu')(x)
            x = Dense(48, activation='relu')(x)
            d_model = 48
        
    elif game_name == "Game-3":
        # Game-3: full IMU + binary features
        # For concatenated: 7 features * 5 placements = 35 features
        if input_features >= 25:  # Multi-placement concatenated (e.g., 35 features)
            x = Dense(192, activation='relu')(x)
            x = Dense(96, activation='relu')(x)
            d_model = 96
        else:  # Single placement (7 features)
            x = Dense(96, activation='relu')(x)
            x = Dense(64, activation='relu')(x)
            d_model = 64
    else:
        # Game-1: Binary data
        # For concatenated: 1 feature * 5 placements = 5 features
        if input_features <= 5:
            x = Dense(32, activation='relu')(x)
            d_model = 32
        else:  # Larger input (unexpected but handle gracefully)
            x = Dense(64, activation='relu')(x)
            d_model = 64
...
def build_hybrid_cnn_transformer(input_shape, num_classes, game_name, featureset='base', use_film=False,
    ...
    if len(input_shape) == 3:  # (time, placements, features)
        # Reshape to 2D for Conv1D: (time, placements * features)
        x_cnn = Reshape((input_shape[0], input_shape[1] * input_shape[2]))(inputs)
    else:  # (time, features) - already 2D
        x_cnn = inputs
    
    x_cnn = Conv1D(64, 5, activation='relu', padding='same')(x_cnn)
    ...
    if game_name == "Game-1" and featureset != 'concatenated':
        # For non-concatenated binary data (e.g., shape (100, 1) or (100, 5, 1))
        embedding_dim = 32
        # Convert inputs to integer for embedding
        inputs_int = tf.keras.layers.Lambda(lambda x: tf.cast(x, tf.int32))(inputs)
        x_transformer = Embedding(input_dim=2, output_dim=embedding_dim)(inputs_int)
        
        # Handle different input shapes
        if len(input_shape) == 3:  # (time, placements, features)
            # Reshape to (time, placements * embedding_dim)
            x_transformer = Reshape((input_shape[0], input_shape[1] * embedding_dim))(x_transformer)
            embedding_dim = input_shape[1] * embedding_dim
        else:  # (time, features)
            x_transformer = Reshape((input_shape[0], embedding_dim))(x_transformer)
    else:
        # For concatenated data or other games, use Dense layer
        # Handle different input shapes first
        if len(input_shape) == 3:  # (time, placements, features)
            # Reshape to 2D for Dense layer: (time, placements * features)
            x_transformer = Reshape((input_shape[0], input_shape[1] * input_shape[2]))(inputs)
        else:  # (time, features) - already 2D
    ...
    print(f"   🔧 Transformer embedding_dim: {embedding_dim}")
    print(f"   🔧 Input shape for positional encoding: seq_len={input_shape[0]}, d_model={embedding_dim}")
    
    # Add positional encoding with unique name to avoid layer reuse issues
    import time
    unique_name = f"pos_encoding_{int(time.time() * 1000000) % 1000000}"
    pos_encoding_layer = ImprovedPositionalEncoding(input_shape[0], embedding_dim, name=unique_name)
    x_transformer = pos_encoding_layer(x_transformer)
...
def train_and_evaluate_advanced_model(X_train, y_train, X_test, y_test, model_type, game_name, split_name, 
    ...
    # Legacy results directory - disabled to prevent duplicate saves
    ...
    # Also get training predictions for saving
    print(f"🎯 Evaluating {model_type} on training set...")
    if use_film and train_user_metadata is not None:
        # Use the final training data (not validation split)
        train_predictions = model.predict([X_train_processed, train_user_metadata])
        if hierarchical:
            y_train_pred = np.argmax(train_predictions[0], axis=1)  # Use fine predictions
        else:
    ...
    # Save the final model to results directory for ensemble use - disabled for individual saving
    ...
    return (y_pred, accuracy, y_train_pred, train_accuracy, train_f1, train_precision, train_recall, 
            y_train, X_train_processed, history, best_epoch_info, has_zero_recall, zero_recall_classes.tolist())

def save_advanced_results(y_pred, y_test, accuracy, y_train_pred, y_train, train_accuracy, 
                         model, history, model_name, game_name, split_name, results_root, 
                         best_epoch_info=None, featureset='base', use_film=False, use_focal_loss=False, 
                         use_label_smoothing=False, label_smoothing=0.0, placement='all'):
    """Save results with advanced metrics and best epoch info for both train and test"""
    ...
    from sklearn.metrics import precision_score, recall_score, f1_score, mutual_info_score, confusion_matrix
    from scipy.stats import entropy
    from collections import Counter
    
    def calculate_metrics_and_kl_div(y_true, y_pred):
        """Calculate all metrics including KL divergence"""
        precision = precision_score(y_true, y_pred, average='weighted')
        recall = recall_score(y_true, y_pred, average='weighted')
        f1 = f1_score(y_true, y_pred, average='macro')  # Use macro-F1 for class-balanced evaluation
        ...
        kl_div = float(entropy(true_probs, pred_probs))
        
        return precision, recall, f1, mi, kl_div, cm
    
    # Calculate test metrics
    test_precision, test_recall, test_f1, test_mi, test_kl_div, test_cm = calculate_metrics_and_kl_div(y_test, y_pred)
    
    # Calculate train metrics  
    train_precision, train_recall, train_f1, train_mi, train_kl_div, train_cm = calculate_metrics_and_kl_div(y_train, y_train_pred)
    ...
    test_metrics = {
        ...
        "mutual_information": float(test_mi),
        "kl_divergence": float(test_kl_div),
    ...
    train_metrics = {
        ...
        "mutual_information": float(train_mi),
        "kl_divergence": float(train_kl_div),
    ...
    if best_epoch_info:
        test_metrics["best_epoch_info"] = best_epoch_info
        train_metrics["best_epoch_info"] = best_epoch_info
    
    # Save metrics to separate folders
    ...
    if model is not None:
        ...
        model.save(model_path)
        # Add model path to both metrics for reference
        test_metrics["model_path"] = model_path
        train_metrics["model_path"] = model_path
    else:
    ...
    print(f"   📁 Train results: {train_dir}")
    ...
    # Return combined metrics for compatibility
    combined_metrics = {
        "train": train_metrics,
        "test": test_metrics,
