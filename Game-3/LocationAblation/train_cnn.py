...
# Disabled for memory management
...
EPOCHS = 15  # Further reduced for memory management
BATCH_SIZE = 16  # Much smaller batch size for memory efficiency
LEARNING_RATE = 0.001
MIN_DELTA = 0.001  # Minimum change in monitored quantity to qualify as an improvement
PATIENCE = 6  # Reduced patience for faster training
...
CHECKPOINT_DIR = os.environ.get("NNEDGE_PRIVACY_CHECKPOINT_ROOT", os.path.join(REPO_ROOT, "checkpoints", "Game-3_CNN_LocationAblation_Checkpoints"))
...
def get_or_create_script_id():
    if os.path.exists(CHECKPOINT_DIR):
        ...
        existing_dirs = [d for d in os.listdir(CHECKPOINT_DIR) 
                        if d.startswith("Game-3_CNN_LocationAblation_") and len(d.split('_')) >= 4]
        ...
        for dir_name in existing_dirs:
            ...
            if os.path.exists(progress_file):
                try:
                    ...
                    print(f"🔍 Found experiment: {dir_name} (Epoch {current_epoch}/{progress.get('total_epochs', 30)}, Status: {status})")
    ...
    new_script_id = f"Game-3_CNN_LocationAblation_{uuid.uuid4().hex[:8]}"
...
def cleanup_empty_checkpoints():
    ...
    for dir_name in os.listdir(CHECKPOINT_DIR):
        if not dir_name.startswith("Game-3_CNN_LocationAblation_"):
            continue
...
def extract_sequences(df):
    ...
    if not all(f in df.columns for f in ALL_FEATURES):
        return np.empty((0, window_size, 13))  # 13 features total after enhancement
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        ...
        if window_data.shape[0] == window_size:
            ...
            # Feature engineering for better HAR performance
            ...
            # 3. Binary shake pattern features
            shake_transitions = np.diff(binary_data.astype(int))
            shake_transitions = np.pad(shake_transitions, (1, 0), 'constant').reshape(-1, 1)
            
            # 4. Movement intensity (acceleration variance)
            movement_intensity = np.var(acc_data, axis=1).reshape(-1, 1)
            
            # Combine all features: 6 (original) + 2 (magnitude) + 2 (mean) + 1 (transitions) + 1 (intensity) + 1 (binary) = 13 features
            enhanced_features = np.column_stack([
                ...
                movement_intensity,          # 1 feature: movement intensity
...
def build_cnn(input_shape, num_classes):
    """
    Memory-optimized CNN architecture for HAR with multimodal sensor data.
    Designed to handle accelerometer, gyroscope, and binary shake detection features.
    """
    with tf.device('/GPU:0' if tf.config.list_physical_devices('GPU') else '/CPU:0'):
        model = Sequential([
            ...
            # Feature processing layers (reduced size)
            Dense(16, activation='tanh', name='feature_preprocessing'),
            BatchNormalization(),
            
            # Simplified convolution block 1: Capture patterns
            Conv1D(32, 3, padding='same', name='conv1'),
            BatchNormalization(),
            Activation('relu'),
            MaxPooling1D(2),
            Dropout(0.2),
            
            # Simplified convolution block 2: Capture medium-term patterns
            Conv1D(64, 3, padding='same', name='conv2'),
            BatchNormalization(),
            Activation('relu'),
            MaxPooling1D(2),
            Dropout(0.3),
            ...
            # Global pooling to capture overall patterns
            GlobalAveragePooling1D(),
            
            # Simplified dense classification layers
        ...
        model.compile(
            ...
            metrics=['accuracy']  # Reduced metrics to save memory
        )
        
        print(f"🏗️ Memory-optimized CNN model built on device: {'GPU' if tf.config.list_physical_devices('GPU') else 'CPU'}")
...
def train_model_with_checkpoints(model, X_train, y_train_cat, X_test, experiment_id, epochs=EPOCHS, batch_size=BATCH_SIZE):
    ...
    def augment_har_sequences(X, y, shift_range=1, noise_std=0.005):
        ...
        for i in range(len(X)):
            ...
            # Only minimal time shifting (50% less augmentation)
            if np.random.random() < 0.3:  # Only 30% chance for augmentation
    ...
    # Minimal augmentation for memory efficiency
    ...
    with tf.device(device_name):
        ...
        if start_epoch < epochs:
            ...
            history = model.fit(
                ...
                verbose=1, 
                callbacks=callbacks, 
                validation_split=0.1,  # Reduced validation split for memory
                shuffle=True,
...
print("🚀 Starting Game-3 Location Ablation Experiments")
print("=" * 60)
# run_lo_location()
