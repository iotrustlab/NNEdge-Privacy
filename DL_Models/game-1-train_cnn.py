# --- FIXED PATCHED FOR COMBINATORIAL SPLITS, CACHING, CHECKPOINTING ---
import os
import re
import glob
import json
...
user_cache_path = os.path.join(get_results_root("Game-1", "CNN"), "user_data_all.pkl")
...
for window_size in window_sizes:
    ...
    results_root = get_results_root("Game-1", "CNN", f"Window_{window_size}")
    ...
    print(f"✅ CNN training for window size {window_size} completed.")

# The remainder of this file is a legacy duplicated implementation block.
# Exit after the maintained path above completes so batch runs do not re-enter it.
sys.exit(0)

# --- PATCHED FOR COMBINATORIAL SPLITS, CACHING, CHECKPOINTING ---
import pickle
import itertools
import matplotlib.pyplot as plt
import seaborn as sns
...
# ---------- CONFIG ----------
data_root = get_data_root()
results_root = get_results_root("Game-1", "CNN")
window_size = get_window_sizes([100])[0]
VALID_ACTIVITIES = {"jogging", "laying", "sitting", "standing", "upstairs", "downstairs", "walking"}
EPOCHS = 20
BATCH_SIZE = 32
os.makedirs(results_root, exist_ok=True)

def extract_sequences(df, window_size_override=None):
    active_window_size = (
        window_size if window_size_override is None else window_size_override
    )
    return np.array([
        df[['dec_tree_out_1']].iloc[i:i + active_window_size].values
        for i in range(0, len(df) - active_window_size + 1, active_window_size)
        if df[['dec_tree_out_1']].iloc[i:i + active_window_size].shape[0]
        == active_window_size
    ])
...
for n_train, n_test in split_configs:
    ...
    combo_files = [f for f in os.listdir(split_path) if f.startswith("combo_") and f.endswith(".json")]
    if len(combo_files) >= max_combinations:
        print(f"✅ {max_combinations} combos already done for {split_name}, skipping.")
        continue
    if len(combos) > max_combinations:
        combos = random.sample(combos, max_combinations)
    metrics_list = []
    ...
    np.random.seed(42)
    random_idx = np.random.randint(0, len(combos)) if combos else 0
    for idx, train_users in enumerate(tqdm(combos, desc="Splits", ncols=80)):
        test_users = [u for u in user_list if u not in train_users]
        if len(test_users) != n_test:
            continue
        # Create a unique filename for this combo
        train_str = "_".join(sorted(train_users))
        test_str = "_".join(sorted(test_users))
        combo_file = os.path.join(split_path, f"combo_{train_str}__{test_str}.json")
        if os.path.exists(combo_file):
        ...
        # Save results for this combo immediately
        with open(combo_file, "w") as f:
            json.dump({
                "train_users": train_users,
                "test_users": test_users,
...
print(f"✅ CNN training for Game-1 completed.")
