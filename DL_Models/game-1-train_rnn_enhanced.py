# --- GAME-1 RNN WITH CLASS-INDEPENDENT METRICS & 2 RANDOM COMBINATIONS ---
import os
import re
import glob
import json
...
from tensorflow.keras.layers import LSTM, GRU, Dropout, Flatten, Dense
...
user_cache_path = os.path.join(get_results_root("Game-1", "RNN"), "user_data_all.pkl")
...
for window_size in window_sizes:
    ...
    results_root = get_results_root("Game-1", "RNN", f"Window_{window_size}")
    ...
    for n_train, n_test in split_configs:
        ...
        for idx, train_users in enumerate(combos):
            ...
            model = build_rnn((window_size, 1), len(le.classes_))
...
print(f"🎉 Game-1 RNN training completed with class-independent metrics!")
