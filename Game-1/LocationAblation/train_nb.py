"""
Game-1 Naive Bayes Script with Sensor Location Ablation Study
...
def extract_features(df):
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        ...
        # Basic statistics
        basic_stats = win.agg(['mean', 'std', 'min', 'max']).values.flatten()
        
        # Additional temporal features
        diff = np.diff(win['dec_tree_out_1'].values)  # First order difference
        zero_crossings = np.sum(np.diff(np.signbit(diff)))  # Rate of change in direction
        peak_freq = len(np.where(np.diff(np.signbit(diff)))[0]) / window_size  # Peak frequency
        
        # Energy and entropy features
        energy = np.sum(np.square(win['dec_tree_out_1'].values)) / window_size
        hist, _ = np.histogram(win['dec_tree_out_1'].values, bins='auto', density=True)
        ...
        features = np.concatenate([
            ...
            [zero_crossings, peak_freq],  # Temporal features
            [energy, entropy]  # Energy and entropy
        ])
