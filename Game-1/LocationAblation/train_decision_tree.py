"""
Game-1 Decision Tree Classifier Script with Sensor Location Ablation Study
...
def train_optimized_model(X_train, y_train):
    """
    Train an optimized decision tree model with hyperparameter tuning and class balancing
    """
...
def extract_features(df):
    feats = []
    df_values = df['dec_tree_out_1'].values
    
    for i in range(0, len(df) - window_size + 1, window_size):
        win = df_values[i:i + window_size]
        ...
        min_val = np.min(win)
        max_val = np.max(win)
        q1 = np.percentile(win, 25)
        q3 = np.percentile(win, 75)
        skew_val = skew(win)
        kurt_val = kurtosis(win)
        median = np.median(win)
        ptp = np.ptp(win)
        
        # Domain-specific features
        zero_crossings = np.sum(np.diff(np.signbit(win)))
        rms = np.sqrt(np.mean(np.square(win)))
        
        # Combine all features
        all_features = np.array([
            mean, std, min_val, max_val,
            q1, q3, skew_val, kurt_val,
            median, ptp, zero_crossings, rms
        ])
        feats.append(all_features)
        ...
        for activity in activities:
            ...
            for loc in locations:
                ...
                for file in files:
                    ...
                    try:
                        # Read CSV with explicit dtype to prevent mixed type issues
                        df = pd.read_csv(file, dtype={'dec_tree_out_1': float})
                        ...
                        df = df.dropna(subset=['dec_tree_out_1'])
                        if len(df) < window_size:
                            print(f"   ⚠️ Not enough data points after cleaning in {file}")
                            continue
                        ...
                        try:
                            ...
                            print(f"   ❌ Feature extraction error in {file}: {feat_error}")
                            continue
