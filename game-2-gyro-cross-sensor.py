#!/usr/bin/env python3
"""
Game-2-Gyro Cross-Sensor Generalization Study with Gyroscope Data
=================================================================

This script implements a placement-specific Game-2 gyroscope adversarial HAR study to test
cross-placement generalization capabilities using gyroscope + binary decision tree
data with user identity available.

✅ AVAILABLE SEMANTIC INFORMATION:
- User identity (for user-specific modeling)
- Raw gyroscope data: (gyro_x, gyro_y, gyro_z)
- Binary decision tree outputs from specific known placements
- Temporal and statistical patterns from gyroscope signals

❌ REMOVED SEMANTIC INFORMATION:
- Accelerometer data (Game-2 gyro constraint: gyroscope only)
...
when using gyroscope + binary data with user identity (Game-2 gyro constraint)?

🔬 EXPERIMENTAL DESIGN:
- Train models on ONE specific placement using gyroscope features
- Test on ALL 5 placements individually
- Compare same-placement vs cross-placement performance
- Hypothesis: Gyroscope features should show different placement specificity vs accelerometer
...
METHODOLOGY:
- Train on specific placement data with gyroscope features + user identity
- Test cross-placement generalization with rich gyroscope feature set
- Compare against Game-2 (accelerometer) and Game-3 (full sensor) results
- Evaluate gyroscope-specific placement characteristics
...
class Game2GyroCrossSensorAnalyzer:
    """
    Analyzer for Game-2 gyroscope cross-sensor generalization study
    """
    
    def __init__(self):
        self.run_config = build_run_config("game-2-gyro-cross-sensor")
        ...
        # Results storage
        self.base_results_dir = str(build_results_dir(self.run_config.results_root, "game-2-gyro-cross-sensor"))
        
        print(f"🎯 Target: Game-2 Gyro Cross-Sensor Generalization Study")
        ...
        print(f"📈 Sensors: Gyroscope + Binary (Game-2 gyro constraint)")
        print(f"🚫 Excluded: Accelerometer data (Game-2 gyro constraint)")
    ...
    def load_user_data_single_placement(self, user_id, placement):
        """
        Load gyroscope + binary data from SPECIFIC placement
    ...
    def extract_gyroscope_features(self, df, user_id):
        """
        Extract rich gyroscope features for Game-2 gyro
        Focus on gyroscope patterns + binary decision tree + user identity
        """
        if len(df) == 0:
            return {}
        ...
        # 1. GYROSCOPE STATISTICAL FEATURES
        for axis_col in self.gyro_cols:
            if axis_col in df.columns:
                ...
                if len(data) > 1:
                    ...
                    # Rotation-specific features
                    features[f'gyro_{axis}_abs_mean'] = np.mean(np.abs(data))
                    features[f'gyro_{axis}_var'] = np.var(data)
        
        # 2. GYROSCOPE MAGNITUDE FEATURES (angular velocity magnitude)
        ...
        # 3. CROSS-AXIS CORRELATION FEATURES (rotation coupling)
        if all(col in df.columns for col in self.gyro_cols):
        ...
        if all(col in df.columns for col in self.gyro_cols):
            ...
            if len(gyro_x) > 1:
                angular_accel_x = np.diff(gyro_x)
                angular_accel_y = np.diff(gyro_y)
                angular_accel_z = np.diff(gyro_z)
                
                features['gyro_angular_accel_x_mean'] = np.mean(angular_accel_x)
                features['gyro_angular_accel_y_mean'] = np.mean(angular_accel_y)
                features['gyro_angular_accel_z_mean'] = np.mean(angular_accel_z)
                
                features['gyro_angular_accel_x_std'] = np.std(angular_accel_x)
                features['gyro_angular_accel_y_std'] = np.std(angular_accel_y)
                features['gyro_angular_accel_z_std'] = np.std(angular_accel_z)
            
            # Dominant rotation axis
            mean_abs_gyro = [np.mean(np.abs(gyro_x)), np.mean(np.abs(gyro_y)), np.mean(np.abs(gyro_z))]
            dominant_axis = np.argmax(mean_abs_gyro)
            features['gyro_dominant_axis'] = dominant_axis
            features['gyro_dominant_axis_strength'] = mean_abs_gyro[dominant_axis]
            
            # Rotation intensity (overall rotational activity)
            features['gyro_rotation_intensity'] = np.mean([np.std(gyro_x), np.std(gyro_y), np.std(gyro_z)])
        
        # 5. BINARY DECISION TREE FEATURES
        ...
        # 6. USER IDENTITY SEMANTIC INFORMATION
            ...
            for activity in self.activities:
                if activity in user_data:
                    ...
                    if len(df) > 0:
                        ...
                        features = self.extract_gyroscope_features(df, user_id)
    ...
    def analyze_placement_specific_split(self, split_name, train_placement):
        ...
        for test_placement in self.all_placements:
            if placement_results[test_placement]:
                ...
                # Print summary
                print(f"\\n📊 GAME-2-GYRO {train_placement}→{test_placement} RESULTS:")
                ...
                print(f"   Features:  {stats['avg_features']:.0f} (gyroscope + binary + user identity)")
                ...
                save_data = {
                    ...
                    'sensor_types': 'gyroscope_binary',
                    'game_type': 'game-2-gyro',
                    'frequency_hz': self.run_config.frequency_hz,
                    'max_combinations_requested': self.run_config.max_combinations,
                }
    ...
    def evaluate_single_combination_all_placements(self, train_users, test_users, train_placement, split_name):
        ...
        try:
            ...
            # Train model - Game-2 gyro optimized Random Forest
    ...
    def _analyze_detailed_progress(self):
        ...
        print(f"\\n📊 DETAILED PROGRESS ANALYSIS (Game-2-Gyro)")
        print("=" * 80)
    ...
    def run_comprehensive_analysis(self):
        ...
        """
        print("🚀 STARTING COMPREHENSIVE GAME-2-GYRO CROSS-SENSOR ANALYSIS")
        print("=" * 80)
        print(f"📂 Results directory: {self.base_results_dir}")
        ...
        print(f"🎯 GYROSCOPE FEATURES: Gyroscope + Binary + User Identity")
...
def main():
    ...
    try:
        # Initialize analyzer
        analyzer = Game2GyroCrossSensorAnalyzer()
