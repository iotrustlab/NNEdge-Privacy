#!/usr/bin/env python3
"""
Game-3 Cross-Sensor Generalization Study with Raw Sensor Data
=============================================================

This script implements a placement-specific Game-3 adversarial HAR study to test
cross-placement generalization capabilities using ALL sensor data (accelerometer + 
gyroscope + binary decision tree) with user identity available.

✅ AVAILABLE SEMANTIC INFORMATION:
- User identity (for user-specific modeling)
- Raw sensor data: accelerometer (acc_x, acc_y, acc_z)
- Raw sensor data: gyroscope (gyro_x, gyro_y, gyro_z)  
- Binary decision tree outputs from specific known placements
- Rich temporal and statistical patterns

🎯 RESEARCH QUESTION:
How well do models trained on one specific placement generalize to other placements
when using FULL sensor data (raw accelerometer + gyroscope + binary) with user identity?

🔬 EXPERIMENTAL DESIGN:
- Train models on ONE specific placement using ALL sensor features
- Test on ALL 5 placements individually
- Compare same-placement vs cross-placement performance
- Hypothesis: Raw sensor features should show stronger placement specificity than binary-only
...
METHODOLOGY:
- Train on specific placement data with FULL sensor features + user identity
- Test cross-placement generalization with rich feature set
- Compare against Game-1 (binary-only) results
- Evaluate whether raw sensors improve placement specificity
...
print("🎭 GAME-3 PLACEMENT-SPECIFIC CROSS-GENERALIZATION STUDY")
print("=" * 65)
print("Testing cross-placement generalization WITH raw sensor data + user identity")
print("Training on single placements | Testing on all placements individually")
print("Using ALL sensor features: Accelerometer + Gyroscope + Binary + User Identity")
print("📊 Evaluation: Traditional + Information-Theoretic Metrics (NMI %, RKL %)")
print()

class Game3CrossSensorAnalyzer:
    """
    Analyzer for Game-3 cross-sensor generalization with full sensor data
    Removes placement information while using rich sensor features + user identity
    """
    
    def __init__(self):
        self.run_config = build_run_config("game-3-cross-sensor")
        ...
        # Results storage
        self.base_results_dir = str(build_results_dir(self.run_config.results_root, "game-3-cross-sensor"))
        
        # Initialize parallel processing
        self.gpu_available = self._enable_gpu_acceleration()
        
        print(f"🎯 Target: Placement-specific Game-3 with raw sensor data + user identity")
        ...
        print(f"📈 Sensors: Accelerometer (3D) + Gyroscope (3D) + Binary Decision Tree")
        print(f"🧪 Cross-placement generalization study with FULL sensor data")
    ...
    def _enable_gpu_acceleration(self):
        """Enable GPU acceleration if available (placeholder for Random Forest)"""
        # Random Forest doesn't use GPU, but we could add GPU-based models later
        return False
    ...
    def load_user_data_single_placement(self, user_id, placement):
        """
        Load accelerometer + gyroscope + binary data from SPECIFIC placement
        This is for placement-specific training (same as Game-2 but with gyroscope)
        """
            ...
            if activity in ['Standing', 'Sitting', 'Laying', 'Walking', 'Jogging']:
                ...
                if os.path.exists(file_path):
                    try:
                        ...
                        missing_cols = [col for col in self.all_sensor_cols if col not in df.columns]
                        if missing_cols:
                            continue
                        
                        # Keep accelerometer + gyroscope + binary columns
                        df_clean = df[self.all_sensor_cols].copy()
    ...
    def extract_full_sensor_features(self, df, user_id):
        """
        Extract comprehensive features from all sensors for Game-3
        Uses exact patterns from Game-2 accelerometer + Game-2-gyro gyroscope features
        """
        if df is None or len(df) == 0:
            return {}
        
        # Check if df has columns attribute and is not None
        if not hasattr(df, 'columns') or df.columns is None:
            return {}
        
        features = {}
        
        # 1. ACCELEROMETER STATISTICAL FEATURES (from Game-2)
        ...
        # 2. ACCELEROMETER MAGNITUDE FEATURES (from Game-2)
        ...
        # 3. ACCELEROMETER CROSS-AXIS CORRELATION FEATURES (from Game-2)
        if all(col in df.columns for col in self.accel_cols):
        ...
        # 4. GYROSCOPE STATISTICAL FEATURES (from Game-2-gyro)
        for axis_col in self.gyro_cols:
            if axis_col in df.columns:
                data = df[axis_col].values
                axis = axis_col.replace('[dps]', '').replace('[mdps]', '').replace('gyro_', '')
                
                # Basic statistics
                features[f'gyro_{axis}_mean'] = np.mean(data)
                features[f'gyro_{axis}_std'] = np.std(data)
                features[f'gyro_{axis}_max'] = np.max(data)
                features[f'gyro_{axis}_min'] = np.min(data)
                features[f'gyro_{axis}_median'] = np.median(data)
                features[f'gyro_{axis}_range'] = np.max(data) - np.min(data)
                features[f'gyro_{axis}_energy'] = np.sum(data**2)
                
                # Advanced statistics
                if len(data) > 1:
                    features[f'gyro_{axis}_skew'] = pd.Series(data).skew()
                    features[f'gyro_{axis}_kurtosis'] = pd.Series(data).kurtosis()
                    features[f'gyro_{axis}_rms'] = np.sqrt(np.mean(data**2))
                    ...
                    # Percentiles
                    features[f'gyro_{axis}_q25'] = np.percentile(data, 25)
                    features[f'gyro_{axis}_q75'] = np.percentile(data, 75)
                    features[f'gyro_{axis}_iqr'] = np.percentile(data, 75) - np.percentile(data, 25)
                    
                    # Zero crossings
                    features[f'gyro_{axis}_zero_crossings'] = np.sum(np.diff(np.sign(data)) != 0)
        
        # 5. GYROSCOPE MAGNITUDE FEATURES (from Game-2-gyro)
        if all(col in df.columns for col in self.gyro_cols):
            ...
            features['gyro_magnitude_range'] = np.max(magnitude) - np.min(magnitude)
            features['gyro_magnitude_energy'] = np.sum(magnitude**2)
            
            if len(magnitude) > 1:
                features['gyro_magnitude_skew'] = pd.Series(magnitude).skew()
                features['gyro_magnitude_kurtosis'] = pd.Series(magnitude).kurtosis()
                features['gyro_magnitude_rms'] = np.sqrt(np.mean(magnitude**2))
        
        # 6. GYROSCOPE CROSS-AXIS CORRELATION FEATURES (from Game-2-gyro)
        if all(col in df.columns for col in self.gyro_cols):
            gyro_x = df[self.gyro_cols[0]].values
            gyro_y = df[self.gyro_cols[1]].values
            gyro_z = df[self.gyro_cols[2]].values
            
            if len(gyro_x) > 1:
                features['gyro_xy_corr'] = np.corrcoef(gyro_x, gyro_y)[0, 1] if not np.isnan(np.corrcoef(gyro_x, gyro_y)[0, 1]) else 0
                features['gyro_xz_corr'] = np.corrcoef(gyro_x, gyro_z)[0, 1] if not np.isnan(np.corrcoef(gyro_x, gyro_z)[0, 1]) else 0
                features['gyro_yz_corr'] = np.corrcoef(gyro_y, gyro_z)[0, 1] if not np.isnan(np.corrcoef(gyro_y, gyro_z)[0, 1]) else 0
        
        # 7. GYROSCOPE ROTATION SPECIFIC FEATURES (from Game-2-gyro)
        if all(col in df.columns for col in self.gyro_cols):
            gyro_data = df[self.gyro_cols].values
            
            # Angular acceleration (second derivative)
            if len(gyro_data) > 2:
                angular_accel = np.diff(gyro_data, n=2, axis=0)
                features['gyro_angular_accel_magnitude_mean'] = np.mean(np.linalg.norm(angular_accel, axis=1))
                features['gyro_angular_accel_magnitude_std'] = np.std(np.linalg.norm(angular_accel, axis=1))
            
            # Dominant rotation axis
            gyro_magnitude_per_axis = np.mean(np.abs(gyro_data), axis=0)
            dominant_axis = np.argmax(gyro_magnitude_per_axis)
            features['gyro_dominant_axis'] = dominant_axis
            features['gyro_dominant_axis_strength'] = gyro_magnitude_per_axis[dominant_axis]
            
            # Rotation consistency
            gyro_magnitude = np.linalg.norm(gyro_data, axis=1)
            if len(gyro_magnitude) > 1 and np.std(gyro_magnitude) > 0:
                features['gyro_rotation_consistency'] = np.mean(gyro_magnitude) / (np.std(gyro_magnitude) + 1e-8)
            else:
                features['gyro_rotation_consistency'] = 0
        
        # 8. BINARY DECISION TREE FEATURES
        ...
        # 9. USER IDENTITY SEMANTIC INFORMATION
    ...
    def create_dataset_single_placement(self, user_ids, placement):
        """Create dataset from single placement for training (same as Game-2 but with gyroscope)"""
        all_features = []
        all_labels = []
        all_metadata = []
        ...
        for user_id in user_ids:
            ...
            for activity in self.activities:
                if activity in user_data:
                    ...
                    if df is not None and len(df) > 0 and hasattr(df, 'columns') and df.columns is not None:
                        # Extract full sensor features
                        features = self.extract_full_sensor_features(df, user_id)
    ...
    def evaluate_cross_placement_combination(self, train_users, test_users, train_placement, test_placement, split_name):
        """Evaluate a single train/test combination for specific placements"""
        try:
            print(f"🔄 Training on {train_placement}, Testing on {test_placement}")
            
            # Create datasets
            X_train, y_train, train_meta = self.create_dataset_single_placement(train_users, train_placement)
            X_test, y_test, test_meta = self.create_dataset_single_placement(test_users, test_placement)
            ...
            model = RandomForestClassifier(
                n_estimators=150,  # Increased for better performance with rich features
                max_depth=15,      # Deeper trees for complex feature interactions
                random_state=42,
                class_weight='balanced',
                n_jobs=-1          # Use all CPU cores
            )
            
            # Train and get training accuracy
            ...
            return {
                ...
                'combination_id': f"{train_users}_{test_users}",
                'has_user_identity': True,
                'sensor_types': 'accelerometer_gyroscope_binary'
            }
            
        except Exception as e:
            print(f"❌ Error in combination {train_placement}→{test_placement}: {e}")
            return None
    ...
    def analyze_placement_specific_split(self, split_name, train_placement):
        ...
        """
        print(f"\n{'='*80}")
        print(f"🎭 TRAINING ON: {train_placement} | SPLIT: {split_name} (Game-3)")
        print(f"{'='*80}")
        ...
        for test_placement in self.all_placements:
            if placement_results[test_placement]:
                ...
                # Extract information-theoretic metrics with backward compatibility
                nmi_percentages = [r.get('nmi_percentage', 0) for r in results]
                reverse_kl_percentages = [r.get('reverse_kl_percentage', 0) for r in results]
                ...
                stats = {
                    ...
                    'nmi_percentage_mean': np.mean(nmi_percentages),
                    'nmi_percentage_std': np.std(nmi_percentages),
                    'reverse_kl_percentage_mean': np.mean(reverse_kl_percentages),
                    'reverse_kl_percentage_std': np.std(reverse_kl_percentages),
                ...
                # Print summary
                print(f"\n📊 GAME-3 {train_placement}→{test_placement} RESULTS:")
                print(f"   Accuracy:  {stats['accuracy_mean']:.3f} ± {stats['accuracy_std']:.3f}")
                print(f"   F1-Score:  {stats['f1_mean']:.3f} ± {stats['f1_std']:.3f}")
                print(f"   NMI %:     {stats['nmi_percentage_mean']:.1f}% ± {stats['nmi_percentage_std']:.1f}%")
                print(f"   RKL %:     {stats['reverse_kl_percentage_mean']:.1f}% ± {stats['reverse_kl_percentage_std']:.1f}%")
                print(f"   Vulnerability: {stats['vulnerability_mean']:.3f} ± {stats['vulnerability_std']:.3f}")
                print(f"   Features:  {stats['avg_features']:.0f} (rich sensor + user identity)")
    ...
    def evaluate_single_combination_all_placements(self, train_users, test_users, train_placement, split_name):
        ...
        try:
            ...
            for test_placement in self.all_placements:
                ...
                test_accuracy, test_f1, test_precision, test_recall, test_nmi_percentage, test_reverse_kl_percentage, vulnerability, n_test_samples = self._test_model_on_placement(
                ...
                if test_accuracy is not None:
                    ...
                    results[test_placement] = {
                        ...
                        'recall': test_recall,
                        'nmi_percentage': test_nmi_percentage,
                        'reverse_kl_percentage': test_reverse_kl_percentage,
    ...
    def _test_model_on_placement(self, model_package, test_users, test_placement):
        ...
        try:
            ...
            if not test_features:
                print(f"⚠️  No test features found for placement {test_placement}, users {test_users}")
            ...
            return accuracy, f1, precision, recall, nmi_percentage, reverse_kl_percentage, vulnerability, len(test_features)
    ...
    def _analyze_detailed_progress(self):
        ...
        print(f"\n📊 DETAILED PROGRESS ANALYSIS (Game-3)")
        print("=" * 80)
    ...
    def _save_split_results_immediately(self, split_name, split_results, split_best_models):
        """Save additional summary results immediately after a split completes - matches Game-1 format"""
        try:
            ...
            # Save best models summary for this split
            print(f"💾 Consolidating best models for {split_name}...")
            models_dir = os.path.join(self.base_results_dir, "best_models")
            os.makedirs(models_dir, exist_ok=True)
            
            # Create overall summary of best models for this split
            split_summary = {
                'split_ratio': split_name,
                'game_type': 'game-3',
                'sensor_types': 'accelerometer_gyroscope_binary',
                'individual_files_verified': individual_files_count,
                'best_models_per_placement': {},
                'timestamp': datetime.now().isoformat()
            }
            
            models_saved = 0
            for train_placement in self.all_placements:
                # Check if we have best models for this training placement
                placement_best_accuracy = 0
                placement_best_file = None
                
                # Look for best model files for this placement
                for test_placement in self.all_placements:
                    model_filename = f"best_model_{train_placement}_to_{test_placement}.joblib"
                    model_filepath = os.path.join(models_dir, split_name, model_filename)
                    
                    if os.path.exists(model_filepath):
                        # Load metadata to find best accuracy
                        metadata_filename = f"best_model_{train_placement}_to_{test_placement}_metadata.json"
                        metadata_filepath = os.path.join(models_dir, split_name, metadata_filename)
                        ...
                        if os.path.exists(metadata_filepath):
                            try:
                                with open(metadata_filepath, 'r') as f:
                                    ...
                                    if accuracy > placement_best_accuracy:
                                        placement_best_accuracy = accuracy
                                        placement_best_file = model_filename
                            except:
                                pass
                
                split_summary['best_models_per_placement'][train_placement] = {
                    'best_accuracy': placement_best_accuracy,
                    'best_model_file': placement_best_file
                }
                
                if placement_best_file:
                    models_saved += 1
            ...
            print(f"💾 Best models tracked: {models_saved}/{len(self.all_placements)} placements")
    ...
    def run_comprehensive_analysis(self):
        ...
        """
        print("🚀 STARTING COMPREHENSIVE GAME-3 CROSS-SENSOR ANALYSIS")
        print("=" * 80)
        print(f"📂 Results directory: {self.base_results_dir}")
        ...
        print(f"🎯 RICH SENSOR FEATURES: Accelerometer + Gyroscope + Binary + User Identity")
        print("=" * 80)
        ...
        for split_name in self.split_ratios.keys():
            ...
            if len(split_results) > 0:
                # Save all results immediately (matches Game-1 format)
...
def main():
    ...
    try:
        # Initialize analyzer
        analyzer = Game3CrossSensorAnalyzer()
