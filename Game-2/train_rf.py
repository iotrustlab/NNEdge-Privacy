...
def compute_leakage_metrics(y_true, y_pred, labels):
    ...
    return {"mutual_information": float(mi), "kl_divergence": float(kl), "confusion_matrix": cm.tolist()}

# ---------- MAIN ----------
for modality in ["accel", "gyro"]:
    print(f"\n🧪 Starting modality: {modality.upper()}")
    ...
    for user in user_dirs:
        ...
        for file_path in csv_files:
            ...
            try:
                ...
                feats = extract_features(df, modality)
                if feats.size == 0:
                    continue
    ...
    for train_ratio, test_ratio in train_test_ratios:
        ...
        if not train_users or not test_users:
            print(f"⚠️ Skipping {split_name}: not enough users")
            continue
        ...
        pd.DataFrame(metrics["confusion_matrix"], index=labels_sorted, columns=labels_sorted).to_csv(os.path.join(split_path, "confusion_matrix.csv"))
    ...
    if all_preds and all_trues:
        ...
        with open(os.path.join(intra_path, "summary.txt"), "w") as f:
            f.write(f"Modality: {modality.upper()} Intra-user Combined Report\n")
        ...
        with open(os.path.join(intra_path, "metrics.json"), "w") as f:
            json.dump(metrics, f, indent=4)
        pd.DataFrame(metrics["confusion_matrix"], index=labels_sorted, columns=labels_sorted).to_csv(os.path.join(intra_path, "confusion_matrix.csv"))
    ...
    if all_preds and all_trues:
        ...
        with open(os.path.join(loo_path, "summary.txt"), "w") as f:
            f.write(f"Modality: {modality.upper()} Leave-One-Out Summary\n")
            f.write(f"Accuracy: {acc:.4f}\n\n{report}")
        labels_sorted = sorted(list(set(all_trues) | set(all_preds)))
        metrics = compute_leakage_metrics(all_trues, all_preds, labels_sorted)
        with open(os.path.join(loo_path, "metrics.json"), "w") as f:
            json.dump(metrics, f, indent=4)
        pd.DataFrame(metrics["confusion_matrix"], index=labels_sorted, columns=labels_sorted).to_csv(os.path.join(loo_path, "confusion_matrix.csv"))

    print(f"✅ Finished {modality.upper()} in {time.time() - start_time:.2f} seconds")
