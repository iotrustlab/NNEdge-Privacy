...
DEFAULT_SUNRGBD_PROCESSED_ROOT = os.path.join(
    os.environ.get(
        "SUNRGBD_PROCESSED_ROOT",
        os.path.join(
...
class TrainA(Dataset):
	def __init__(self, path = DEFAULT_SUNRGBD_PROCESSED_ROOT, complete = False):
...
class FinetuneDataset(Dataset):
	def __init__(self, path = DEFAULT_SUNRGBD_PROCESSED_ROOT):
		self.img = []
		self.semseg = [] #  Get semseg only if I pass in complete
		self.depth = []
		self.labels = []
...
# 	with open(DEFAULT_SUNRGBD_PROCESSED_ROOT + "labels.txt") as handle:
# 		lines = [line.rstrip() for line in handle]
...
# 	torch.save(train_A, DEFAULT_SUNRGBD_PROCESSED_ROOT + "trainA.pt")
# 	torch.save(train_B, DEFAULT_SUNRGBD_PROCESSED_ROOT + "trainB.pt")
# 	torch.save(finetune, DEFAULT_SUNRGBD_PROCESSED_ROOT + "finetune.pt")
# 	torch.save(test, DEFAULT_SUNRGBD_PROCESSED_ROOT + "test.pt")
