...
results_root = os.path.join(data_root, "Results", "Game-3", "DT")
window_size = 20
...
def extract_features(df):
    ...
    for i in range(0, len(df) - window_size + 1, window_size):
        win = df[ALL_FEATURES].iloc[i:i + window_size]
...
# ---------- MAIN ----------
print(f"\n🚀 Starting Game-3 using ALL sensor data + dec_tree")
...
if all_preds and all_trues:
    ...
    with open(os.path.join(loo_path, "summary.txt"), "w") as f:
        f.write(f"Leave-One-Out Summary\nAccuracy: {acc:.4f}\n\n{report}")
    labels_sorted = sorted(list(set(all_trues) | set(all_preds)))
    metrics = compute_leakage_metrics(all_trues, all_preds, labels_sorted)
...
print(f"✅ Game-3 completed in {time.time() - start_time:.2f} seconds")
