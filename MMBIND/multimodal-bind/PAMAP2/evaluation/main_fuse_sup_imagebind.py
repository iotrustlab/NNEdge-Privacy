...
PAMAP2_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_MAG_WEIGHT = PAMAP2_ROOT / "train" / "save_imagebind" / "save_train_B_contrastive_no_load_acc_100_split_0" / "models" / "lr_0.0001_decay_0.0001_bsz_64" / "last.pth"
DEFAULT_GYRO_WEIGHT = PAMAP2_ROOT / "train" / "save_imagebind" / "save_train_A_contrastive_no_load_acc_100_split_0" / "models" / "lr_0.0001_decay_0.0001_bsz_64" / "last.pth"
...
def set_model(opt):
    ...
    mag_weight = os.environ.get("PAMAP2_IMAGEBIND_MAG_WEIGHT", str(DEFAULT_MAG_WEIGHT))
    model_template_mag.load_state_dict(torch.load(mag_weight)['model'])
    model.mag_encoder = model_template_mag.mag_encoder
    
    gyro_weight = os.environ.get("PAMAP2_IMAGEBIND_GYRO_WEIGHT", str(DEFAULT_GYRO_WEIGHT))
